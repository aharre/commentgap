...
## Load dataset
```{r}
comment_data_general <- read.csv("comment_data_general.csv")
...
# load dataset
comment_data_general_features <- read.csv("comment_data_general_features.csv")
...
##regression user votes -> Python file 

##prepare dataset
```{r}
# X - modify dataset
comment_features_ml <- comment_data_general_features[,c(# intrinsic quality
...
all_indices <- 1:nrow(comment_features_ml)
all_indices <- sample(all_indices) #randomise

training_indices <- all_indices[1:round(length(all_indices)*0.6,0)] # 60% 
test_indices <- all_indices[round(length(all_indices)*0.6,0):length(all_indices)] # 40%
```
...
summary(comment_data_general_1$cosine_1)
...
new_data <- comment_data_general_1[,c("sentiment_prob_pos", "sentiment_prob_neg", "lexdiv_cttr",
                                      "num_punct", "num_sentences", "SMOG_readability", "cosine_1",
                                      "hours_since_article", "is_leaf_comment", "size_of_tree",
                                      "height_of_tree", "all_replies", "second_person", "user_follower",
                                      "votes_pos", "votes_neg")]

pin_pred_lr <- predict(model1_2, newdata = new_data, type = "response")
...
##classifier editors picks 
```{r}

comment_features_4 <- comment_data_general[,c("word_to_sent", "SMOG_readability", "num_words_unique", "char_to_sent",
                                            "lexdiv_cttr", "num_punct",
                                            "num_words", "num_sentences", "num_char",
                                            
                                            "is_root_comment", "is_leaf_comment", "size_of_tree", "height_of_tree",
                                            "level_in_tree",
...
head(comment_features_4)

# REMOVE ALL NA rows ## -> double check if this is OK
nrow(comment_features_4)
comment_features_4 <- na.omit(comment_features_4)

# Indices of training, validation, and test observations 
all_indices <- 1:nrow(comment_features_4)
all_indices <- sample(all_indices) #randomise

training_indices <- all_indices[1:round(length(all_indices)*0.5,0)] # 60% 
validation_indices <- all_indices[round(length(all_indices)*0.5,0):round(length(all_indices)*0.6,0)] # 20%
test_indices <- all_indices[round(length(all_indices)*0.6,0):length(all_indices)] # 20%
...
# Estimation - 14:15 - 
model_rf_4 <- randomForest(pinned_f~., data=training_dataset, mtry = round(sqrt(ncol(training_dataset)-1), 0),
                        importance=TRUE)

# Prediction
test_y_hat_prob_rf <-  predict(model_rf_4, newdata = test_X, type="prob")[,2]
test_y_hat_rf <- predict(model_rf_4, newdata = test_X)

# Confusion matrix
confm <- confusionMatrix(data = test_y, reference = test_y_hat_rf, positive = "TRUE", mode = "everything")
precision <- confm$table[4] / (confm$table[4] + confm$table[2])
recall <- confm$table[4] / (confm$table[4] + confm$table[3])
...
```{r}
varImpPlot(model_rf_4, main = "Variable importance")

rf_4_imp <- data.frame(importance(model_rf_4))
rf_4_imp$model <- "pinned"
...
#Add predicted values to dataset for new sorting algorithm
```{r}
# read predicted values
library(readr)
up_vote_pred_nb <- read_csv("C:/Users/flora/OneDrive - London School of Economics/Thesis_Social Navigation/Thesis/up_vote_pred.csv")
up_vote_pred_rf <- read_csv("C:/Users/flora/OneDrive - London School of Economics/Thesis_Social Navigation/Thesis/up_vote_pred_rf.csv", col_names = FALSE)
#pin_pred_lr
pin_pred_rf <- read_csv("C:/Users/flora/OneDrive - London School of Economics/Thesis_Social Navigation/Thesis/pin_pred_rf.csv", col_names = FALSE)
colnames(pin_pred_rf) <- c("prob_notpinned", "prob_pinned")
pin_pred_rf <- pin_pred_rf[2]

#comment_data_general_2 <- comment_data_general

# add to data frame
comment_data_general$up_vote_pred_nb <- up_vote_pred_nb$`0`
comment_data_general$up_vote_pred_rf <- up_vote_pred_rf$X1
comment_data_general$pin_pred_lr <- pin_pred_lr
comment_data_general$pin_pred_rf <- pin_pred_rf$prob_pinned
